#!/usr/bin/env bash
#
# Run the iOS Quickstart E2E suite end to end: start a ThunderID server, provision the test
...
build_sample() {
    ...
    # The sample reaches the self-signed local server because its Info.plist carries an
    # NSAllowsArbitraryLoads exemption.
    cat > "$sample_dir/Sources/Config.plist" <<PLIST
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
	<key>THUNDERID_BASE_URL</key>
	<string>$SERVER_URL</string>
	<key>THUNDERID_APPLICATION_ID</key>
...
PLIST
    ...
    xcrun simctl install "$SIM_UDID" \
        "$sample_dir/.build/e2e-derived/Build/Products/Debug-iphonesimulator/Quickstart.app"
}
